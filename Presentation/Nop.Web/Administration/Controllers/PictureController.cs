...
namespace Nop.Admin.Controllers
{
    public partial class PictureController : BaseController
    {
        ...
        public ActionResult AsyncUpload()
        {
         ...
         [HttpPost]
        public ActionResult AsyncSupportPictureUpload()
        {
            //if (!_permissionService.Authorize(StandardPermissionProvider.UploadPictures))
            //    return Json(new { success = false, error = "You do not have required permissions" }, "text/plain");
            ...
            //otherwise some browsers will pop-up a "Save As" dialog.
            return Json(new { success = true, pictureId = picture.Id,
                imageUrl = _pictureService.GetPictureUrl(picture, 200) },
                "text/plain");
        }
        
        [HttpPost]
        public ActionResult AsyncMultiImageUpload()
        {
            //if (!_permissionService.Authorize(StandardPermissionProvider.UploadPictures))
            //    return Json(new { success = false, error = "You do not have required permissions" }, "text/plain");
            ...
            {
                Session["MultiImageUplode"] = Session["MultiImageUplode"].ToString() + "+" + picture.Id;
            }
            else {
                Session["MultiImageUplode"] = picture.Id;
            }
        ...
        public ActionResult ChangeSession(int id)
        {
            if (Session["MultiImageUplode"] != null)
            {
                ...
                for(int i=0;i< pictureids.Length;i++)
                {
                    if (id != pictureids[i])
                    {
                        Session["MultiImageUplode"] = (Session["MultiImageUplode"] != null) ? (Session["MultiImageUplode"].ToString() + "+" + (pictureids[i].ToString())) : pictureids[i].ToString();
