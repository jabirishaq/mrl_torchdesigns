...
namespace Nop.Admin.Controllers
{
    public partial class NewsController : BaseAdminController
	{
        ...
        public ActionResult Comments(int? filterByNewsItemId ,int id=0)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageNews))
                return AccessDeniedView();
            ...
            if (id != 0)
            {
                var newscomment = _newsService.GetNewsCommentById(id);
                newscomment.IsApproved = true;
                _newsService.ApprovedComment(newscomment);
        ...
        [HttpPost]
        public ActionResult Comments(int? filterByNewsItemId, DataSourceRequest command,int commentid = 0)
          {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageNews))
                return AccessDeniedView();
            ...
            var gridModel = new DataSourceResult
            {
                Data = comments.PagedForCommand(command).Select(newsComment =>
                {
                    ...
                    commentModel.Isapprove = newsComment.IsApproved;
        ...
        public ActionResult CommentUpdate(NewsCommentModel model)
        {
            ...
            var newscomment = _newsService.GetNewsCommentById(model.Id);
            newscomment.IsApproved = model.Isapprove;
            _newsService.ApprovedComment(newscomment);
