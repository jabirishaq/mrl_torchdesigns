...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        public ProductController(ISpecificationAttributeGroupService specificationgroupservice,ICategoryService categoryService, 
            IManufacturerService manufacturerService,
            IProductService productService,
            IProductServiceCustome productservicecustome,
            ...
            ICacheManager cacheManager,IProductThumbService productthumbService)
        {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            //var modelMaximumRuns = new MaximumRunsModel()
            //{
                
            //    ShowMaximumRuns_Td = _productservicecustome.GetMaximumRunsByProductId();
            ...
            var customefield = _productservicecustome.GetCustomeFieldByProductId(product.Id);
            //model.ShowMaximumRuns = customefield.ShowMaximumRuns;
            model.RunId = customefield.RunId;
            model.MadeInUsa = customefield.MadeInUsa;
            model.DisplayQuantity = customefield.Quantity;
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                foreach (var picture in productPictures)
                {
                    ...
                    {
                        DefaultPictureImageUrl = _pictureService.GetPictureUrl(picture.PictureId, defaultPictureSize, !isAssociatedProduct),
                        ImageUrl = (_productthumbService.Getthumbbypictureid(picture.PictureId) != null) ? (_productthumbService.Getthumbbypictureid(picture.PictureId).ThumbId > 0 ? _pictureService.GetPictureUrl(_productthumbService.Getthumbbypictureid(picture.PictureId).ThumbId,_mediaSettings.ProductThumbPictureSizeOnProductDetailsPage) : "") : "",
                        FullSizeImageUrl = _pictureService.GetPictureUrl(picture.PictureId, 0),
                        Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), model.Name),
                        AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name),
                    });
                }
                ...
                ////all pictures
                //var picturesthumb = _pictureService.GetPicturesthumbByProductId(product.Id);
                //var pictureModels = new List<PictureModel>();
                //foreach (var picture in picturesthumb)
                //{
                //    pictureModels.Add(new PictureModel()
                //    {  DefaultPictureImageUrl = _pictureService.GetPictureUrl(picture.PictureId, defaultPictureSize, !isAssociatedProduct),
                //        ImageUrl = _pictureService.GetPictureUrl(picture.ThumbId, _mediaSettings.ProductThumbPictureSizeOnProductDetailsPage),
                //       FullSizeImageUrl = _pictureService.GetPictureUrl(picture.PictureId, 0),
                //        Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), model.Name),
                //        AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name),
                //    });
            ...
            if (!isAssociatedProduct)
            {
                model.ProductSpecifications.Add(this.PrepareProductSpecificationModel(_workContext,
        ...
        public ActionResult ProductDetails(int productId, int updatecartitemid = 0)
        {
            ...
            //Product Detail Page  product custome field
        ...
        public ActionResult ProductVideo(int productId, int? productThumbPictureSize)
        {
            ProductVideos model = new ProductVideos();

            var productvideo = _productservicecustome.GetProductVideoByProductId(productId);
            foreach(var item in productvideo)
            {
                model.Videolist.Add(new Videolist
                {   
                    Id=item.Id,
                    PictureId=item.PictureId,
                    PictureUrl = _pictureService.GetPictureUrl(item.PictureId, 200),
                    VideoUrl=(item.VideoId).Trim()
                 });
            }
        ...
        public ActionResult ProductDownload(int productId)
        {
            var down = _productservicecustome.GetProductDownloadByProductId(productId);
            ProductDownloads model = new ProductDownloads();
            model.ProductId = productId;
            foreach (var item in down)
            {
                model.Downloadlist.Add(new Downloadlist
                {
                    Id = item.Id,
                    Title = item.DownloadTitle,
                    DownloadId = item.DownloadId,
                    Description = item.DownloadDescription
                });
            }
            return PartialView(model);
        }

        public ActionResult ProductInBox(int productId)
        { // int quant=0;
            var box = _productservicecustome.GetProductFromBoxByPerantProductId(productId);
            ProductIntoBox model = new ProductIntoBox();
            foreach (var item in box)
           {
               Productlistinbox md = new Productlistinbox();
               ...
               {   var product=_productService.GetProductById(item.InboxProductId);
                   md.Title = item.Title;
                   md.Sename = product.GetSeName();
                   md.Type = 1;
                   md.Id = item.Id;
                   md.Partno = product.ManufacturerPartNumber;
                  // int.TryParse(product.Quantity, out quant);
                   md.Quantity = item.Quantity; 
               }

               model.Productlistinbox.Add(md);
        ...
        public ActionResult ProductReviewsAdd(int productId, ProductReviewsModel model, bool captchaValid)
        {
            ...
            {
                //save review
                int rating = Convert.ToInt32(model.AddProductReview.RatingSelected);
