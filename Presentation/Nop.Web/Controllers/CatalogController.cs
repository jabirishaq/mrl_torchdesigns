...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public CatalogController(IOrderReportService orderReportService,ICategoryService categoryService, 
            ...
            bool allowCustomersToSelectPageSize, string pageSizeOptions, int fixedPageSize)
        {
            ...
            { int pageall=int.MaxValue;
            var page = pageall.ToString() + ", " + pageSizeOptions;
                var pageSizes = page.Split(new char[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
                ...
                if (pageSizes.Any())
                {
                    ...
                    foreach (var pageSize in pageSizes)
                    {
                        ...
                        var temppage = "";
                        if (pageSize == int.MaxValue.ToString())
                        {
                            temppage = "All";
                        }
                        else
                    ...
                    {
                        pagingFilteringModel.PageSizeOptions = pagingFilteringModel.PageSizeOptions.OrderBy(x => int.Parse((x.Text=="All") ? (int.MaxValue.ToString()):x.Text )).ToList();
        ...
        public ActionResult MainProductCategory()
        {
            var customerRolesIds = _workContext.CurrentCustomer.CustomerRoles
                .Where(cr => cr.Active).Select(cr => cr.Id).ToList();
            var allcategory = _categoryService.GetAllCategories().Where(x => x.ParentCategoryId == 0);

            var Category = new ProductCategoryModel();
           foreach(var c in allcategory)
           {
               var model = new ProductCategoryModel.productClickCategoryModel() 
               {
                   Name = c.Name,
                   Id = c.Id,
                   SeName = c.GetSeName(),
                   CategoryImageurl = _pictureService.GetPictureUrl(c.PictureId,_mediaSettings.CategoryThumbPictureSize)
               };
               Category.ProductCategories.Add(model);
           }
        ...
        public ActionResult TopMenu()
        {
            ...
            var model = new TopMenuModel()
            {
               ...
               SystemDesignEnabled = _storeInformationSettings.ShowSystemDesign
                // SupportEnabled = _catalogSettings.SupportEnabled
            };
            return PartialView(model);
        }
        ...
        #endregion
        #region Watering Tips
        public ActionResult Wateringtip()
        {
        ...
        #region Download PDF Instructions
        public ActionResult DownloadPDFInstruction()
        {
