...
namespace Nop.Web.Controllers
{
    ...
    public partial class BlogController : BasePublicController
    {
        ...
        protected virtual void PrepareBlogPostModel(BlogPostModel model, BlogPost blogPost, bool prepareComments)
        {
            ...
            int bcommentcount = _blogService.GetAllCommentsByBlogPostId(blogPost.Id).Where(x => x.IsApproved == true).Count();
            ...
            model.NumberOfComments = bcommentcount;
            model.AddNewComment.DisplayCaptcha = _captchaSettings.Enabled && _captchaSettings.ShowOnBlogCommentPage;
            if (prepareComments)
            {
                var blogComments = blogPost.BlogComments.Where(x=>x.IsApproved==true).OrderBy(pr => pr.CreatedOnUtc);
        ...
        protected virtual BlogPostListModel PrepareBlogPostListModel(BlogPagingFilteringModel command)
        {
            ...
            model.BlogPosts = blogPosts
                .Select(x =>
                {
                    ...
                    PrepareBlogPostModel(blogPostModel, x, true);
        ...
        public ActionResult BlogCommentAddOnlist(int blogPostId, string blogPostText)
        {
            if (!_blogSettings.Enabled)
                return RedirectToRoute("HomePage");
            ...
            //validate CAPTCHA
            if (_captchaSettings.Enabled && _captchaSettings.ShowOnBlogCommentPage )
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                _workflowMessageService.SendBlogCommentNotificationMessageWithActivationCode(comment, _localizationSettings.DefaultAdminLanguageId);
