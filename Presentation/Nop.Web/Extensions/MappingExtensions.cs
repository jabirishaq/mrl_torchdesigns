    ...
    public static class MappingExtensions
    {
            ...
            Customer customer = null)
        {
            ...
            if (!excludeProperties && address != null)
            {
                ...
                {
                    var usaCountries = countryService.GetAllUsaCountryCustome().Select(x => x.Id).ToArray();
                    var USA = int.MaxValue;
                    if (address.CountryId.HasValue)
                        if (usaCountries.Contains(address.CountryId.Value))
                        {
                            model.CountryId = USA;
            ...
            if (address == null && prePopulateWithCustomerFields)
            {
                ...
                //model.StateId = customer.GetAttribute<int>(SystemCustomerAttributeNames.StateId);
            ...
            if (addressSettings.CountryEnabled && loadCountries != null)
            {
                ...
                model.AvailableCountries.Add(new SelectListItem() { Text = "USA", Value = int.MaxValue.ToString() });
                ...
                if (addressSettings.StateProvinceEnabled)
                {
                    ...
                    var USA = int.MaxValue;
                    model.CountryId = model.CountryId == null ? USA : model.CountryId;
                    model.AvailableStates.Add(new SelectListItem() { Text = localizationService.GetResource("Address.SelectState"), Value = "0" });
                    if (model.CountryId == USA)
                    {
                        var states = stateProvinceService.GetAllStateProvincesOfUSA();
            ...
            model.AddressType = address != null && !string.IsNullOrEmpty(address.Company)?"C":"I";
        }
