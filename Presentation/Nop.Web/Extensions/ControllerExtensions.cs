...
namespace Nop.Web.Extensions
{
    ...
    public static class ControllerExtensions
    {
        ...
        public static ProductSpecificationModel PrepareProductSpecificationModel(this Controller controller,
            ...
            Product product)
        {
            ...
            var specificationgroupservice = EngineContext.Current.Resolve<ISpecificationAttributeGroupService>();

            var model = specificationAttributeService.GetProductSpecificationAttributesByProductId(product.Id, null, true);

            var specification = new ProductSpecificationModel();
          
            //IList<ProductSpecificationModel> specifications = new List<ProductSpecificationModel>(); 
            foreach (var item in model)
            {
                if (specification.Grouplist.Where(x => x.Groupid == item.SpecificationAttributeGroupId).FirstOrDefault() == null)
                { 
                var productSpecificationAttributes = specificationAttributeService.GetSpecificationByGroupid(item.SpecificationAttributeGroupId).Where(x=> item.ProductId == x.ProductId);
                var groupList = new Grouplist();
              
                    groupList.Groupid = item.SpecificationAttributeGroupId;
                    foreach (var spec in productSpecificationAttributes)
                    {

                        groupList.SpecificatioAttribute.Add(new SpecificatioAttribute()
                        {
                            SpecificationAttributeId = spec.SpecificationAttributeOptionId,
                            SpecificationAttributeName = specificationAttributeService.GetSpecificationAttributeById(specificationAttributeService.GetSpecificationAttributeOptionById(spec.SpecificationAttributeOptionId).SpecificationAttributeId).Name,
                            SpecificationAttributeOption = specificationAttributeService.GetSpecificationAttributeOptionById(spec.SpecificationAttributeOptionId).Name
                        });
                    }
                    var sag = specificationgroupservice.GetSpecificationGroupById(item.SpecificationAttributeGroupId);
                    if (sag != null)
                        groupList.GroupName = sag.Name;
                    specification.Grouplist.Add(groupList);
                }
           }
         ...
         )
        {
            ...
            foreach (var product in products)
            {
                ...
                {
                    model.SpecificationAttributeModels.Add(PrepareProductSpecificationModel(controller, workContext,
