...
namespace Nop.Plugin.Payments.AuthorizeNet
{
    ...
    public class AuthorizeNetPaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        private readonly IAuthorizeNetCardDeclinedLogService _authorizeNetCardDeclinedLogService;
        private readonly IWorkContext _workContext;
        private readonly AuthorizeNet_ObjectContext _objectContext;
        ...
        public AuthorizeNetPaymentProcessor(AuthorizeNetPaymentSettings authorizeNetPaymentSettings,
            ...
            IAuthorizeNetCardDeclinedLogService authorizeNetCardDeclinedLogService,
            IWorkContext workContext,
            ILogger logger,
            IOrderProcessingService orderProcessingService,
            IOrderService orderService,
            ILocalizationService localizationService,
            AuthorizeNet_ObjectContext objectContext)
        {
            ...
            this._authorizeNetCardDeclinedLogService = authorizeNetCardDeclinedLogService;
        ...
        private void AddDeclinedCardLogAndBannedIP(string errorMessage)
        {
            var getCreditCardDeclineLogs = _authorizeNetCardDeclinedLogService.SearchAuthorizeNetCardDeclinedLogs(_workContext.CurrentCustomer.Id, DateTime.UtcNow.Date);
            if (getCreditCardDeclineLogs.Count >= 4)
            {
                var currentIPaddress = _webHelper.GetCurrentIpAddress();
                _authorizeNetCardDeclinedLogService.AddIPToBannedList(currentIPaddress, _workContext.CurrentCustomer);
            }

            var cardDeclinedLog = new Td_AuthorizeNet_DeclinedCard_Log()
            {
                CreatedOn = DateTime.UtcNow.Date,
                CreditCardDeclinedDueTo = !string.IsNullOrEmpty(errorMessage) ? errorMessage : string.Empty,
                CustomerId = _workContext.CurrentCustomer.Id
            };
            _authorizeNetCardDeclinedLogService.InsertAuthorizeNetCardDeclinedLog(cardDeclinedLog);
        }
        ...
        public ProcessPaymentResult ProcessPayment(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            var responseError = result.Errors;

            if (responseError.Count > 0)
            {
                foreach (var error in responseError)
                {
                    AddDeclinedCardLogAndBannedIP(error);
                }
            }
            //validate
            if (response == null)
