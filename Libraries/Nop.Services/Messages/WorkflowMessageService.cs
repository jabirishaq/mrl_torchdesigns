...
namespace Nop.Services.Messages
{
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public WorkflowMessageService(IMessageTemplateService messageTemplateService,
            ...
            IEventPublisher eventPublisher, NewsSettings newsSettings)
        {
            ...
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            ...
            //condition added as per task when free shipping is enabled then shipping method is FLAT RATE instead of UPS.GROUND
            if (order.OrderShippingExclTax == decimal.Zero)
                order.ShippingMethod = "Flat Rate";
        ...
        public virtual int SendShipmentSentCustomerNotification(Shipment shipment, int languageId)
        {
          ...
          var  _shippingSettings = EngineContext.Current.Resolve<ShippingSettings>();
           var _shippingService = EngineContext.Current.Resolve<IShippingService>();
            ...
            if (srcm != null &&  srcm.PluginDescriptor.Installed &&
                srcm.IsShippingRateComputationMethodActive(_shippingSettings))
            {
                var shipmentTracker = srcm.ShipmentTracker;
                if (shipmentTracker != null)
                    trackingUrl = shipmentTracker.GetUrl(shipment.TrackingNumber);
            }
                //email account
                var emailAccount = GetEmailAccountOfMessageTemplate(messageTemplate, languageId);
                ...
                var tokens = new List<Token>();
                _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
                _messageTokenProvider.AddShipmentTokens(tokens, shipment, languageId, trackingUrl);
        ...
        /// <summary>
        /// Sends a New blog notification message to a store owner
        /// </summary>
        /// <param name="blogComment">Blog Post</param>
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendNewBlogPostNotificationMessage(NewsLetterSubscription subscription, string email,BlogPost blogpost, int languageId,string seName)
        {
            if (blogpost == null)
            ...
            var messageTemplate = GetActiveMessageTemplate("Blog.SendBlogEmail", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddBlogPostTokens(subscription,tokens, blogpost,seName);
        ...
        /// <summary>
        /// Sends a news comment notification message to a store owner with activation id
        /// </summary>
        /// <param name="newsComment">News comment</param>
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendNewsCommentNotificationWithActivationCode(NewsComment newsComment, int languageId)
        {
            if (newsComment == null)
                throw new ArgumentNullException("newsComment");
            ...
            var messageTemplate = GetActiveMessageTemplate("News.NewsCommentWithActivationCode", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddNewsCommentTokens(tokens, newsComment);
            _messageTokenProvider.AddNewsCommentTokensForActivationcode(tokens, newsComment);
            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);

            var toEmail = _newsSettings.EmailIdforreciveNewsComment;
            var toName = emailAccount.DisplayName;
            return SendNotification(messageTemplate, emailAccount,
                languageId, tokens,
                toEmail, toName);
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendBlogCommentNotificationMessageWithActivationCode(BlogComment blogComment, int languageId)
        {
            ...
            var messageTemplate = GetActiveMessageTemplate("Blog.SendBlogEmailWithActivationCode", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddBlogPostTokensForActivatioCode(tokens, blogComment);
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual int SendSystemDesignHelpNotification(SystemDesignHelp systemdesign, int languageId)
        {

            if (systemdesign == null)
                throw new ArgumentNullException("SystemDesign");

            var store = _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplate("SystemDesignHelp", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddSystemHelpTokens(tokens, systemdesign);

            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);

            var toEmail = "sales@misterlandscaper.com";
            var toName = "";
            var filename="";
            var replaytoname="";
            return SendNotification(messageTemplate, emailAccount,
                languageId, tokens,
                toEmail, toName, systemdesign.PictureUrl, filename, systemdesign.Email, replaytoname);
        ...
        #region Credit card decline notification
        ...
        /// <param name="customer">Customer</param>
        /// <param name="languageId">Message Language Identifier</param>
        /// <param name="toEmailIds">"To" email addresses</param>
        /// <param name="errorMessage">Error message</param>
        /// <returns></returns>
        public virtual int SendCreditCardDeclineMessage(Customer customer, int languageId,
             string toEmailIds, string errorMessage)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");

            if (string.IsNullOrEmpty(toEmailIds))
                throw new ArgumentNullException("toEmailIds");
            ...
            var messageTemplate = GetActiveMessageTemplate("CreditCard.DeclineMessage", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddCustomerTokens(tokens, customer);
            tokens.Add(new Token("CreditCardDecline.ErrorMessage", errorMessage, true));

            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);

            var toEmail = toEmailIds;
            var toName = "";
            return SendNotification(messageTemplate, emailAccount,
                languageId, tokens,
                toEmail, toName);
