...
namespace Nop.Services.Messages
{
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        public virtual void AddShipmentTokens(IList<Token> tokens, Shipment shipment, int languageId, string trackingUrl = null)
        {
            ...
            if (!string.IsNullOrEmpty(trackingUrl))
                tokens.Add(new Token("Shipment.TrackingUrl", trackingUrl,true));
        ...
        public virtual void AddCustomerTokens(IList<Token> tokens, Customer customer)
        {
            ...
            tokens.Add(new Token("Customer.NewPassword", customer.GetAttribute<string>(SystemCustomerAttributeNames.PasswordRecoveryToken)));
            ...
            string passwordRecoveryUrl = string.Format("{0}login", GetStoreUrl());
        ...
        public virtual void AddBlogPostTokens(NewsLetterSubscription subscription, IList<Token> tokens, BlogPost blogpost,string seName)
        {
            tokens.Add(new Token("BlogPost.Title", blogpost.Title));
            string newbody = Regex.Replace(blogpost.Body, "&amp;", "&");
            string body = Regex.Replace(newbody, "&nbsp;", " ");
            //string string1 = Regex.Replace(blogpost.Body, @"(<img\/?[^>]+>)", @"", RegexOptions.IgnoreCase);
            //string string2 = Regex.Replace(string1, "<(.|\n)*?>", "");
            //string string3 = Regex.Replace(string2, "\r\n", "");
            //
            ...
            //    if (body.Length >= 400)
            //    {
            //        tokens.Add(new Token("BlogPost.PostBody", body.Substring(0, 400) + "..."));
            //    }
            //    else
            //    {
            //        tokens.Add(new Token("BlogPost.PostBody", body));
            //    }
            //}
            tokens.Add(new Token("BlogPost.PostBody", body));
            tokens.Add(new Token("BlogPost.Sename", seName));

            //event notification
            _eventPublisher.EntityTokensAdded(blogpost, tokens);
        }
        ...
        public virtual void AddNewsCommentTokensForActivationcode(IList<Token> tokens, NewsComment newsComment)
        {
            tokens.Add(new Token("NewsComment.Id", newsComment.Id.ToString()));
            tokens.Add(new Token("NewsComment.Comment", newsComment.CommentText));
            tokens.Add(new Token("NewsComment.Title", newsComment.CommentTitle));
            //event notification
            _eventPublisher.EntityTokensAdded(newsComment, tokens);
        }
        public virtual void AddBlogPostTokensForActivatioCode(IList<Token> tokens, BlogComment blogComment)
        {
            tokens.Add(new Token("BlogComment.Id", blogComment.Id.ToString()));
            tokens.Add(new Token("BlogComment.Comment", blogComment.CommentText));
        ...
        public virtual void AddSystemHelpTokens(IList<Token> tokens, SystemDesignHelp systemdesign)
        {
            tokens.Add(new Token("SystemHelp.customerfirstname", systemdesign.FirstName.ToString()));
            tokens.Add(new Token("SystemHelp.customerlastname", systemdesign.LastName.ToString()));
            tokens.Add(new Token("SystemHelp.Email", systemdesign.Email.ToString()));
            tokens.Add(new Token("SystemHelp.Watersource", systemdesign.CityWellId.ToString()));
            tokens.Add(new Token("SystemHelp.Countryname", systemdesign.CountryId.ToString()));
            tokens.Add(new Token("SystemHelp.Drippers", systemdesign.DrippersId.ToString()));
            tokens.Add(new Token("SystemHelp.Availablefaucets", systemdesign.HaveFaucetsId.ToString()));
            tokens.Add(new Token("SystemHelp.PSI", systemdesign.PSI.ToString()));
            tokens.Add(new Token("SystemHelp.Second", systemdesign.Second.ToString()));
            tokens.Add(new Token("SystemHelp.Soiltype", systemdesign.SoilTypeId.ToString()));
            tokens.Add(new Token("SystemHelp.Wantfaucents", systemdesign.UseFaucetsId.ToString()));
            tokens.Add(new Token("SystemHelp.Image", systemdesign.PictureUrl.ToString()));
        }
