...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
        ...
        /// <param name="endTimeUtc">End date</param>
        /// <returns>Result</returns>
        public virtual IList<OrderByStateReportLine> GetStateReport(int storeId, OrderStatus? os,
            PaymentStatus? ps, ShippingStatus? ss, DateTime? startTimeUtc, DateTime? endTimeUtc)
        {
            int? orderStatusId = null;
            if (os.HasValue)
            ...
            var report = (from oq in query
                          group oq by oq.BillingAddress.StateProvinceId into result
                          select new
                          {
                              StateProvinceId = result.Key,
                              TotalOrders = result.Count(),
                              SumOrders = result.Sum(o => o.OrderTotal)
                          }
                       )
                       .OrderByDescending(x => x.SumOrders)
                       .Select(r => new OrderByStateReportLine()
                       {
        ...
        public virtual IPagedList<Product> SalesRepeort(int pageIndex, int pageSize) { List<Product> lst = new List<Product>(); var products = new PagedList<Product>(lst, 0, 15); return products; }

        public virtual IPagedList<ProductItem> SalesRepeortItem(DateTime? createdFromUtc, DateTime? createdToUtc, int pageIndex, int pageSize, int? OrderStatusId, int? PaymentStatusId)
        {
            var query22 = (from prodcutItem in _productRepository.Table
                           join o in _orderItemRepository.Table on prodcutItem.Id equals o.ProductId into ops
                           from o in ops.DefaultIfEmpty()
                           join oo in _orderRepository.Table on o.OrderId equals oo.Id into orp
                           from oo in orp.DefaultIfEmpty()
                           where (!createdFromUtc.HasValue || createdFromUtc.Value <= oo.CreatedOnUtc) &&
                                (!createdToUtc.HasValue || createdToUtc.Value >= oo.CreatedOnUtc)
                           select new ProductItem
                           {
                               TotalProduct = (from oi in _orderItemRepository.Table
                                               join or in _orderRepository.Table on oi.OrderId equals or.Id
                                               where (oi.ProductId == o.ProductId)
                                                   && (OrderStatusId > 0 ? or.OrderStatusId == OrderStatusId : or.OrderStatusId == or.OrderStatusId)
                                                   && (PaymentStatusId > 0 ? or.PaymentStatusId == PaymentStatusId : or.PaymentStatusId == or.PaymentStatusId)
                                               select oi).Count(),
                               Name = prodcutItem.Name,
                               Id = prodcutItem.Id

                           }).Distinct().OrderByDescending(x => x.TotalProduct);



            List<ProductItem> lst = new List<ProductItem>();
            lst = query22.ToList();
            var products = new PagedList<ProductItem>(lst, pageIndex, pageSize);

            return products;

        }

        public virtual IPagedList<ProductItem> SoldItemList(DateTime? createdFromUtc, DateTime? createdToUtc, int pageIndex, int pageSize, int startTime, int EndTime)
        {
            int OrderStatusId = 30;
            List<ProductItem> lst = new List<ProductItem>();
            var hours = (createdToUtc.Value - createdFromUtc.Value).TotalHours;
            for (int i = 0; i < hours; i++)
            {
                DateTime? countstartTime;
                DateTime? countEndTime;
                if (i <= hours - 1)
                {
                    countstartTime = createdFromUtc.Value.AddHours(i);
                    countEndTime = createdFromUtc.Value.AddHours(i + 1);
                    var query = (from orderItem in _orderItemRepository.Table
                                 join o in _orderRepository.Table on orderItem.OrderId equals o.Id
                                 where (!countstartTime.HasValue || countstartTime.Value <= o.CreatedOnUtc) &&
                                 (!countEndTime.HasValue || countEndTime.Value >= o.CreatedOnUtc) &&
                                 (o.OrderStatusId == OrderStatusId)
                                 select new ProductItem
                                 {
                                     TotalProduct = (from oi in _orderItemRepository.Table where oi.Id == orderItem.Id 
                                                         select oi).Count(),
                                     StartTime = o.CreatedOnUtc,
                                     Id = orderItem.Id,
                                 }).Distinct();
                        ...
                        obj.Name = Convert.ToString(countstartTime.Value.ToShortDateString() + " " + countstartTime.Value.ToShortTimeString() + " - " + countEndTime.Value.ToShortTimeString());
                        obj.TotalProduct = query.Count();
                        lst.Add(obj);
