...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            discountAmountExclTax = (decimal.Truncate(100 * discountAmountExclTax) / 100);
        ...
        public virtual bool IsFreeShipping(IList<ShoppingCartItem> cart, string Shippingmethodname = null)
        {
            Customer customer = cart.GetCustomer();
            if (customer != null)
            {
            ...
            if (!string.IsNullOrEmpty(Shippingmethodname))
            {
                if (Shippingmethodname.ToLower() == "flat rate")
                {
        ...
        public virtual decimal AdjustShippingRate(decimal shippingRate,
            IList<ShoppingCartItem> cart, out Discount appliedDiscount, string Shippingmethodname = null)
        {
            appliedDiscount = null;
            ...
            {
                if (IsFreeShipping(cart, Shippingmethodname))
                    return decimal.Zero;
            }
            else 
           {
