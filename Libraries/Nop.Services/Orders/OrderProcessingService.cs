...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        public OrderProcessingService(IOrderService orderService,
            ...
            CurrencySettings currencySettings,IGenericAttributeService genericAttributeService)
        {
        ...
        protected virtual void SetActivatedValueForPurchasedGiftCards(Order order, bool activate)
        {
            ...
            foreach (var gc in giftCards)
            {
                if (activate)
                {
                    ...
                    if (!order.Customer.GetAttribute<bool>(SystemCustomerAttributeNames.GetOrderNotification))
                    {
        ...
        public virtual void CheckOrderStatus(Order order)
        {
            ...
            if (order.OrderStatus == OrderStatus.Transmitted)
            {
                if ((order.ShippingStatus == ShippingStatus.Shipped || order.ShippingStatus == ShippingStatus.NotYetShipped) &&
                    (order.PaymentStatus == PaymentStatus.Authorized || order.PaymentStatus == PaymentStatus.Paid))
                {
                    SetOrderStatus(order, OrderStatus.Transmitted, false);
                }
            }
                ...
                order.OrderStatus != OrderStatus.Complete)
            {
                if (order.PaymentStatus == PaymentStatus.Paid)
                {
                    ...
                    {
                        var customer=order.Customer;
                        SetOrderStatus(order, OrderStatus.Complete, customer.GetAttribute<bool>(SystemCustomerAttributeNames.GetOrderNotification));
                    }
                }
            }
        }
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                    ...
                    //using (var scope = new System.Transactions.TransactionScope())
                    {
                       ...
                       #region Notifications & notes
                        ...
                        //send email notifications
                            ...
                            bool Notificationallow = true;
                            var a = order.Customer.GetAttribute<bool>(SystemCustomerAttributeNames.GetOrderNotification);
                            if (!order.Customer.GetAttribute<bool>(SystemCustomerAttributeNames.GetOrderNotification))
                            {
                                if (order.Customer.IsGuest())
                                { Notificationallow = true; }
                        ...
                        //if (processPaymentRequest.Reqirevoid)
                        //{
                        //    var response = _paymentService.Void(new VoidPaymentRequest() { Order=order });
                        //}
                    }
                }
                else
                {
                    ...
                    {
                        if (paymentError == "cvv50")
                        {
                            result.AddError("cvv50");
                        }
                        else if (paymentError == int.MaxValue.ToString())
                        {
                    ...
                    //if (processPaymentResult.AvsResult != null)
                    //{
                    //    if (processPaymentResult.AvsResult == int.MaxValue.ToString())
                    //    {
                    //        result.AddError(int.MaxValue.ToString());
                    //    }
