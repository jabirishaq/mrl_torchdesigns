...
namespace Nop.Services.Catalog
{
    ...
    public partial class CopyProductService : ICopyProductService
    {
        ...
        public CopyProductService(IProductService productService,
            ...
            IProductThumbService productThumbService)
        {
            ...
            bool isPublished = true, bool copyImages = true, bool copyAssociatedProducts = true)
        {
            ...
            if (copyImages)
            {
                foreach (var productPicture in product.ProductPictures)
                {
                    ...
                    var productThumb = _productThumbService.Getthumbbypictureid(picture.Id);
                   if(productThumb != null)
                    {
                        var thumbPicture = _pictureService.GetPictureById(productThumb.ThumbId);
                        if (thumbPicture != null)
                        {
                            var thumbCopy = _pictureService.InsertPicture(_pictureService.LoadPictureBinary(thumbPicture),
                                thumbPicture.MimeType, _pictureService.GetPictureSeName(newName),
                                true);
                            _productThumbService.InsertProductThumbPicture(new ProductThumb()
                            {
                                PictureId = pictureCopy.Id,
                                ProductId = productCopy.Id,
                                ThumbId = thumbCopy.Id
                            });
                        }
                    }
                }
            ...
            foreach (var productSpecificationAttribute in product.ProductSpecificationAttributes)
            {
                var psaCopy = new ProductSpecificationAttribute()
                {
                    ...
                    SpecificationAttributeGroupId = productSpecificationAttribute.SpecificationAttributeGroupId
                };
