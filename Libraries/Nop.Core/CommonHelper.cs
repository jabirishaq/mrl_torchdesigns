...
namespace Nop.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        /// <summary>
        /// format address to replace "&" with "and"
        /// </summary>
        public static Address FormatAddress(Address address)
        {
            if (!string.IsNullOrEmpty(address.Address1))
            {
                address.Address1 =  address.Address1.Replace("&", "and");
            }
            if (!string.IsNullOrEmpty(address.Address2))
            {
                address.Address2 = address.Address2.Replace("&", "and");
            }
            if (!string.IsNullOrEmpty(address.City))
            {
                address.City = address.City.Replace("&", "and");
            }
            if (!string.IsNullOrEmpty(address.Company))
            {
                address.Company = address.Company.Replace("&", "and");
            }
            if (!string.IsNullOrEmpty(address.FirstName))
            {
                address.FirstName = address.FirstName.Replace("&", "and");
            }
            if (!string.IsNullOrEmpty(address.LastName))
            {
                address.LastName = address.LastName.Replace("&", "and");
            }
